...
const getFilteredItems = (query: string, items?: IMenuItem[]) => {
  if (!query) {
    return items;
  }
...
const Items = (props: any) => {
  ...
  const [menuItems, setMenuItems] = useState<IMenuItem[]>(props.menuItems);
  ...
  useEffect(() => {
    const fetchMenuItems = async () => {
      setMenuItems(flatten(props.menuItems));
    };
    props.fetchData()
    fetchMenuItems();
  }, []);
    ...
    <div className="container">
      <div>
        <div className="row">
          ...
          <div className="col-3">
            <Link
              to={`/items/create/${props.menuId}`}
              state={{ categories: props.categories }}
            >
              <button className="btn btn-secondary">
                <i className="bi bi-plus-lg"></i> New item
              </button>
            </Link>
          </div>
        </div>
