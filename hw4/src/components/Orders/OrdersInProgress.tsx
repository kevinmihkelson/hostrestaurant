...
const OrdersInProgress = (props: { restaurantId: string | undefined }) => {
  const [orders, setOrders] = useState<IOrder[]>();
  const orderService = new OrderService();

  const handleConfirm = async (id: string) => {
    await orderService.patch(id, 2);
    await fetchdata();
  };
  ...
  return (
    <div className="container">
      <h1 className="m-3">Orders in progress</h1>
      <hr />
      {orders?.length == 0 && (
        <div className="container mt-5">
          <h4>No orders in progress</h4>
        </div>
      )}
      {orders?.map(
        (order) =>
